    ...
    ui(new Ui::ApplyFriend),_label_point(2,6)
{
    ...
    ui->lb_ed->setMaxLenght(21);
	ui->lb_ed->move(2, 2);
    ui->lb_ed->setFixedHeight(25);  // "搜索、添加标签"框大小调整
...
{
    _si = si;
    auto applyname = applyname_prefix + UserinfoGroup::GetInstance()->getCurrUserName();
...
void ApplyFriend::ShowMoreLabel()
{
    ...
    for(int i = 0; i < _tip_data.size(); i++){
		...
		QFontMetrics fontMetrics(lb->font()); // 获取QLabel控件的字体信息
        int textWidth = fontMetrics.horizontalAdvance(lb->text()); // 获取文本的宽度; // 获取文本的宽度
		int textHeight = fontMetrics.height(); // 获取文本的高度
...
void ApplyFriend::addLabel(QString name)
{
    ...
    // 清空下一个lb_ed 这次的输入已经在tmplabel->show();了，下次应该默认显示搜索提示
    ui->lb_ed->clear();
...
void ApplyFriend::SlotChangeFriendLabelByTip(QString lbtext, ClickLabelState state)
{
    ...
    if(state == ClickLabelState::SELECTED){
        //点击现有标签 添加
        qDebug() << "SlotChangeFriendLabelByTip addLabel";
        addLabel(lbtext);
        return;
    }
    ...
    if(state == ClickLabelState::NORMAL){
        //再点击现有标签 删除
        qDebug() << "SlotChangeFriendLabelByTip removeFriendLabel";
...
{
    qDebug()<<"ApplyFriend 确认添加" ;
    //发送请求逻辑
    QJsonObject jsonObj;
    // 自己uid与名称
    auto uid = UserinfoGroup::GetInstance()->getCurrUid();
    jsonObj["uid"] = uid;
    ...
    // ⚠️对方备注也发过去，方便对方同意了我们能知道是谁
    auto bakname = ui->back_ed->text();
    if(bakname.isEmpty()){
        bakname = ui->back_ed->placeholderText();
    }

    jsonObj["bakname"] = bakname;   // 对方
    jsonObj["touid"] = _si->_uid;   // ⚠️uid用于查找所在服务器：看看对方和哪个chat服务器ip绑定
    ...
    //发送tcp请求给chat server
    qDebug() << "发送添加好友请求";
    emit TcpManager::GetInstance()->signal_send_data(ReqId::ID_ADD_FRIEND_REQ, jsonData);
    this->hide();
    deleteLater();
}
